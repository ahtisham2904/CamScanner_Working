...
/* ---------- OpenCV loader (with timeout so it can't hang) ---------- */
let cvPromise = null;
...
/* ---------- document detection ----------
   Prefers a clean convex quad; falls back to a rotated-rectangle
   fit on the largest contour so "Auto detect" returns something
  ...
  try {
    ...
    for (let i = 0; i < contours.size(); i++) {
      ...
      if (approx.rows === 4 && cv.isContourConvex(approx) && area > quadArea) {
        ...
        quad = [];
        for (let k = 0; k < 4; k++)
          quad.push({ x: approx.data32S[k * 2] / scale, y: approx.data32S[k * 2 + 1] / scale });
      }
      if (area > fbArea) {
        fbArea = area;
...
export function warpBilinear(srcCanvas, corners, maxDim = 1600) {
  ...
  const od = oImg.data;
  const sample = (x, y, c) => (x < 0 || y < 0 || x >= sw || y >= sh) ? 255 : sd[(y * sw + x) * 4 + c];
...
export function magicColorPro(d, w, h) {
  ...
  const contrast = 1.18, sat = 1.12;
  for (let p = 0, i = 0; p < w * h; p++, i += 4) {
    const ch = [d[i], d[i + 1], d[i + 2]].map((v, c) => {
      let nv = (v / Math.max(bg[c][p], 1)) * 235;
      nv = ((nv / 255 - 0.5) * contrast + 0.5) * 255;
